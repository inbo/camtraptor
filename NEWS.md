...
- `get_record_table()` is now deprecated. Use `camtrapR_recordTable()` (#240).
- `get_cam_op()` is now deprecated. Use `camtrapR_cameraOperation()` (#239).
- `get_detection_history()` is now deprecated. Use `camtrapR_detectionHistory()` (#365).
- `get_species()` is now deprecated. Use `taxa()` (#343).
- `get_n_species()` is now deprecated. Use `summarize_observations()` (#243).
- `get_n_obs()` is now deprecated. Use `summarize_observations()` (#367).
- `get_n_individuals()` is now deprecated. Use `summarize_observations()` (#367).
- `get_rai()` is now deprecated. Use `summarize_observations()` (#243).
- `get_rai_individuals()` deprecated. Use `summarize_observations()` (#243).
- `get_custom_effort()` is now deprecated. Use `summarize_deployments()` (#366).
- `get_effort()` is now deprecated. Use `summarize_deployments()` (#366).
- `map_dep()` is now deprecated. Use `map_summary()` (#231, #91).
- `calc_animal_pos()` is now deprecated. Use `calculate_individual_radius_angle()` (#242).
...
- `example_dataset()` allows to create an example dataset in Camtrap DP 1.0 format. It reexports the functionality from [camtrapdp](https://inbo.github.io/camtrapdp/reference/example_dataset.html). The internal dataset `mica` is not present anymore. Raw data files in `inst/extdata` are also removed.
- `deployments()`, `observations()` and `media()` allow to extract the respective tables (#317). They reexports the functionality from camtrapdp, e.g. see [`camtrapdp::deployments()`](https://inbo.github.io/camtrapdp/reference/deployments.html).
- `deployments<-`, `observations<-` and `media<-` allow to set the respective tables (#328). They reexports the functionality from camtrapdp, e.g. see [`camtrapdp::deployments<-`](https://inbo.github.io/camtrapdp/reference/deployments.html).
- `events()`and `taxa()` allow to extract information about events and taxa from the observations (#317). They reexport the functionalities from camtrapdp, see [`camtrapdp::events()`](https://inbo.github.io/camtrapdp/reference/events.html) and [camtrapdp::taxa()](https://inbo.github.io/camtrapdp/reference/taxa.html).
- `locations()` allows to extract information about locations from the deployments (#317). It reexports the functionality from camtrapdp, see [`camtrapdp::locations()`](https://inbo.github.io/camtrapdp/reference/locations.html).
- The first argument of many functions accepts a Camtrap DP object: not `package` anymore, but `x` (#324).
- `filter_deployments()`, `filter_observations()` and `filter_media()` allow to filter at data package level (#315). They reexport the functionalities from camtrapdp. See [`camtrapdp::filter_deployments()`](https://inbo.github.io/camtrapdp/reference/filter_deployments.html), [`camtrapdp::filter_observations()`](https://inbo.github.io/camtrapdp/reference/filter_observations.html) and [`camtrapdp::filter_media()`](https://inbo.github.io/camtrapdp/reference/filter_media.html).
