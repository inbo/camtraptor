...
This vignette shows you how to use the function
[`map_summary()`](https://inbo.github.io/camtraptor/reference/map_summary.md)
to visualize important features for each deployment on an interactive
leaflet map such as:
...
- RAI (Relative Abundance Index) (calculated based on observations or
  individuals)
...
Before making a visualization with
[`map_summary()`](https://inbo.github.io/camtraptor/reference/map_summary.md),
the Camera Trap Data Package dataset should be transformed to a grouped
data frame. This can be done by using the function
...
depending on what aspect of the dataset you want to visualize. Both are
...
Let’s also create a Camera Trap Data Package dataset that is filtered
for a specific species.

To do so, you can first inspect which species have been detected. The
function [`taxa()`](https://inbo.github.io/camtraptor/reference/taxa.md)
gives you an overview of the unique scientific names within the Camera
Trap Data Package dataset:
...
This example filters for Anas platyrhynchos. You can filter by using the
...
You can visualize the number of detected species by each deployment by
using the function
[`map_summary()`](https://inbo.github.io/camtraptor/reference/map_summary.md)
with `feature` set to `n_scientificName`:
...
You can also visualize the number of observations for a specific
species:
...
When the argument `extend` is set as `TRUE` within the
[`summarize_observations()`](https://inbo.github.io/camtraptor/reference/summarize_observations.md)
function, the deployments without observations for Anas platyrhynchos
will also be visualized by a black multiplication symbol `×`:
...
As for observations, you can also visualize the number of observed
individuals for a specific species:
...
> **Note:** Notice that in this package the RAI is normalized over a
> deployment activity period of 100 days.

To visualize the Relative Abundance Index (RAI) for a given species
(e.g. Anas platyrhynchos), set `feature = "rai_observations"`. This RAI
is based on the number of observations, see the next section on how to
calculate the RAI on the number of detected individuals.
...
You can visualize the duration of the deployments, also called *effort*,
as number of active hours by using the grouped data frame created by
[`summarize_deployments()`](https://inbo.github.io/camtraptor/reference/summarize_deployments.md)
in the section ‘Setup’. Set `feature = "effort_duration"`:
...
Other units than ‘hour’ (which is the default) can also be used, by
using the argument `effort_unit`. For example, you can express the
effort in days:
...
species, these two situations are shown by default as black and red
multiplication symbols `×` respectively:
...
To hide deployments with unknown species or deployments with no
observations, set `na_values_show` or `zero_values_show` to `FALSE`
respectively.

### Clustering and hovering

You can control which variables are displayed when hovering over a
deployment using the `hover_columns` argument. You can choose among all
columns from the grouped data frame created with
[`summarize_observations()`](https://inbo.github.io/camtraptor/reference/summarize_observations.md)
or
[`summarize_deployments()`](https://inbo.github.io/camtraptor/reference/summarize_deployments.md).

In the example below only the deploymentID and the number of
observations are shown while hovering:
...
palette](https://CRAN.R-project.org/package=viridis) called `"inferno"`.
You can specify another viridis color palette, e.g. `"viridis"` or
`"magma"`, or a
[RColorBrewer](https://renenyffenegger.ch/notes/development/languages/R/packages/RColorBrewer/index)
palette, e.g. `"BuPu"` or `"Oranges"`. Below the `viridis` color palette
is used:
...
You can customize the icon shown for zero-value deployments by passing
an icon URL to `zero_values_icon_url` and its size in pixels to
`zero_values_icon_size`. Many free icon libraries are available online,
such as [icons8](https://icons8.com/). The following example uses a
custom icon with a size of 50 pixels:
...
Typically the colour is part of the URL. Here below is an example where
you change the color of the default icon to green (2ECC71):
...
By default the upper limit of the color palette and radius are defined
based on the actual feature values. However, sometimes it can be useful
to set up an absolute upper limit. This can be done by setting argument
