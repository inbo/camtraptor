...
- animal_positions:
  ...
  contain (at least) the columns defined in args `deployment_id`,
  `event_id`, `x`, `y`, `image_width` and `image_height`.
...
- deployment_id:
  ...
  Column in `animal_positions` against which names of the elements can
  be matched to apply the right deployment calibration models. Default:
  `"deploymentID"`.

- event_id:

  Column in `animal_positions` containing the event ID the images belong
  to. Default: `"eventID"`.

- x:

  Column in `animal_positions` containing x pixel positions for each
  digitised point. Default: `"x"`.

- y:

  Column in `animal_positions` containing y pixel positions for each
  digitised point. Default: `"y"`.

- image_width:

  Column in `animal_positions` containing the pixel x dimension of each
  image. Default: `"imageWidth"`. Notice that the pixel x dimension must
  be consistent for each deployment.

- image_height:

  Column in `animal_positions` containing the pixel y dimension of each
  image. Default: `"imageHeight"`. Notice that the pixel y dimension
