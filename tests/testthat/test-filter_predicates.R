test_that("right class for all filter predicates", {
  ...
  expect_s3_class(pred_in(
    arg = "a",
    value = c(1, 2)
  ),
  class = "filter_predicate"
  )
  expect_s3_class(pred_notin(
    arg = "a",
    value = c(1, 2)
  ),
  class = "filter_predicate"
...
test_that("specific tests: pred_and and pred_or", {
  ...
  expect_equal(and_pred$arg, list(
    basic_pred$arg,
    in_pred$arg,
    lt_pred$arg,
    notna_pred$arg
  ))
  expect_equal(and_pred$value, list(
    basic_pred$value,
    in_pred$value,
    lt_pred$value,
    notna_pred$value
  ))
  expect_equal(and_pred$type, list(
    basic_pred$type,
    in_pred$type,
    lt_pred$type,
    notna_pred$type
  ...
  expect_equal(or_pred$arg, list(
    basic_pred$arg,
    in_pred$arg,
    lt_pred$arg,
    notna_pred$arg
  ))
  expect_equal(or_pred$value, list(
    basic_pred$value,
    in_pred$value,
    lt_pred$value,
    notna_pred$value
  ))
  expect_equal(or_pred$type, list(
    basic_pred$type,
    in_pred$type,
    lt_pred$type,
    notna_pred$type
...
test_that("specific tests: nesting pred_and and pred_or", {
  ...
  nested_pred <- pred_and(
    pred_and(basic_pred, in_pred),
