...
test_that("output matrix has locations as rownames", {
  cam_op_matrix <- get_cam_op(camtrapdp)
  locations <- paste0("Station", camtrapdp$deployments$location_name)
  n_locations <- length(camtrapdp$deployments$location_name)
...
test_that("output matrix has all days of deployment activity as colnames", {
  cam_op_matrix <- get_cam_op(camtrapdp)
  days_activity <- seq(as.Date(min(camtrapdp$deployments$start)),
                        as.Date(max(camtrapdp$deployments$end)),
                       by = "days")
...
test_that("daily effort is > 0 for active days, 0 for inactive days", {
  cam_op_matrix <- get_cam_op(camtrapdp)
  location <- camtrapdp$deployments$location_name[4]
  deployment_start <- min(camtrapdp$deployments %>%
                             filter(location_name == location) %>%
                             pull(start))
  deployment_end <- max(camtrapdp$deployments %>%
                             filter(location_name == location) %>%
                             pull(end))
...
test_that("daily effort is > 0 and < 1 for partial active days (start/end)", {
  cam_op_matrix <- get_cam_op(camtrapdp)
  location <- camtrapdp$deployments$location_name[4]
  start <- as.character(as.Date(camtrapdp$deployments$start[4]))
  end <- as.character(as.Date(camtrapdp$deployments$end[4]))
