testthat::test_that("predict_pos returns errors if animal_pos is not valid", {
  # animal_pos is not a dataframe
  testthat::expect_error(predict_pos(1, list(a = "a")))
  # x and y columns missing
  testthat::expect_error(
    predict_pos(dplyr::tibble(deployment = "A",
                              ImageWidth = 5,
                              ImageHeight = 10),
                list(a = "a")),
  ...
  testthat::expect_error(
    predict_pos(dplyr::tibble(width = 5,
                              height = 10,
                              x = 4, y = 2),
                list(a = "a")),
...
testthat::test_that(
  "predict_post returns errors if calib_models is not valid", {
  ...
  testthat::expect_error(predict_pos(df, calib_models = 2))
  testthat::expect_error(predict_pos(df, calib_models = list(2)),
...
testthat::test_that("Deployments with no matching calibration model", {
  ...
  testthat::expect_warning(
    predict_pos(animal_positions, missing_calib_model),
...
testthat::test_that("Deploys with multiple values for image width/height", {
  ...
  testthat::expect_warning(
    predict_pos(multi_pixel_dim, dep_calib_models),
...
testthat::test_that("Right output", {
  output <- predict_pos(animal_positions, dep_calib_models)
  # right class
