...
test_that("output is a list", {
  ...
  dp <- suppressMessages(read_camtrap_dp(file = dp_path))
  expect_true(rlang::is_bare_list(dp))
  expect_true(rlang::is_bare_list(dp_v1_rc1))
  expect_named(
    dp,
    expected = c(
      "name",
...
test_that("output data slot is a list of length 3", {
  ...
  dp <- suppressMessages(read_camtrap_dp(file = dp_path))
  rlang::is_bare_list(dp_v1_rc1$data)
  expect_length(dp_v1_rc1$data, 3)
  rlang::is_bare_list(dp$data)
  expect_length(dp$data, 3)
})
...
test_that("datapackage data elements are named as in resource names", {
  ...
  expect_true(all(names(dp_v1_rc1$data) %in% resource_names))
  resource_names <- frictionless::resources(dp_v1_rc1)
    expect_named(dp_v1_rc1, expected = resource_names)
})
...
test_that("datapackage resources are tibble dataframes", {
  ...
  expect_s3_class(dp$data$observations, "data.frame")
  # check for v1.0-rc1
  expect_s3_class(dp_v1_rc1$data$deployments, "tbl_df")
  expect_s3_class(dp_v1_rc1$data$deployments, "tbl")
  expect_s3_class(dp_v1_rc1$data$deployments, "data.frame")
  expect_s3_class(dp_v1_rc1$data$observations, "tbl_df")
  expect_s3_class(dp_v1_rc1$data$observations, "tbl")
  expect_s3_class(dp_v1_rc1$data$observations, "data.frame")
  expect_s3_class(dp_v1_rc1$data$media, "tbl_df")
  expect_s3_class(dp_v1_rc1$data$media, "tbl")
  expect_s3_class(dp_v1_rc1$data$media, "data.frame")
})
...
test_that(
  "v1.0-rc.1: sc. names and vernacular names in obs match info in metadata", {
    taxon_infos <- purrr::map_dfr(
      ...
      dplyr::tibble()
    expect_true(
      all(names(taxon_infos) %in% names(dp_v1_rc1$data$observations))
    )
    # get scientific names from observations and check that they match with
    # taxonomic info
    sc_names <- dp_v1_rc1$data$observations$scientificName[!is.na(
      dp_v1_rc1$data$observations$scientificName
    )]
    expect_true(all(sc_names %in% taxon_infos$scientificName))
    
    # get vernacular names in English from observations and check that they match
    # with taxonomic info
    en_names <- dp_v1_rc1$data$observations$vernacularNames.eng[
      !is.na(dp_v1_rc1$data$observations$vernacularNames.eng)
    ]
    expect_true(all(en_names %in% taxon_infos$vernacularNames.eng))
    
    # get vernacular names in Dutch from observations and check that they match
    # with taxonomic info
    nl_names <- dp_v1_rc1$data$observations$vernacularNames.nld[
      !is.na(
        dp_v1_rc1$data$observations$vernacularNames.nld
      )
    ]
...
test_that(
  ...
  expect_true("latitude" %in% names(dp_v1_rc1$data$deployments))
  expect_true("longitude" %in% names(dp_v1_rc1$data$deployments))
  which(names(dp_v1_rc1$data$deployments) == "latitude") ==
    which(names(dp_v1_rc1$data$deployments) == "longitude") + 1
})

test_that("read deployments v1.0-rc1: eventStart is renamed as start", {
  expect_false("eventStart" %in% names(dp_v1_rc1$data$deployments))
  expect_true("start" %in% names(dp_v1_rc1$data$deployments))
})

test_that("read deployments v1.0-rc1: eventEnd is renamed as end", {
  expect_false("eventEnd" %in% names(dp_v1_rc1$data$deployments))
  expect_true("end" %in% names(dp_v1_rc1$data$deployments))
})

test_that(
  "read deployments v1.0-rc1: cameraDelay is renamed as cameraInterval", {
    expect_false("cameraDelay" %in% names(dp_v1_rc1$data$deployments))
    expect_true("cameraInterval" %in% names(dp_v1_rc1$data$deployments))
})

test_that(
  "read deployments v1.0-rc1: detectionDistance is a new term and is ignored", {
    expect_false(
      "detectionDistance" %in% names(dp_v1_rc1$data$deployments)
    )
})

test_that(
  "read deployments v1.0-rc1: baitUse is a factor, not a boolean", {
    expect_s3_class(dp_v1_rc1$data$deployments$baitUse, "factor")
    baitUse_levels <- c("none", "scent", "food", "visual", "acoustic", "other")
    expect_identical(
      levels(dp_v1_rc1$data$deployments$baitUse), baitUse_levels
    )
    # boolean NA becomes a factor NA
    expect_true(all(is.na(dp_v1_rc1$data$deployments$baitUse)))
  }
)

test_that("read deployments v1.0-rc1: session is left empty", {
  expect_true(all(is.na(dp_v1_rc1$data$deployments$session)))
})

test_that("read deployments v1.0-rc1: array is left empty", {
  expect_true(all(is.na(dp_v1_rc1$data$deployments$array)))
})

test_that("read deployments v1.0-rc1: deploymentTags is renamed as tags", {
  expect_false(
    "deploymentTags" %in% names(dp_v1_rc1$data$deployments)
  )
  expect_true("tags" %in% names(dp_v1_rc1$data$deployments))
})

test_that(
  "read deployments v1.0-rc1: deploymentComments is renamed as comments", {
    expect_false(
      "deploymentComments" %in% names(dp_v1_rc1$data$deployments)
    )
    expect_true("comments" %in% names(dp_v1_rc1$data$deployments))
  }
)

test_that("read deployments v1.0-rc1: _id is left empty", {
  expect_true(all(is.na(dp_v1_rc1$data$deployments$`_id`)))
})
...
test_that(
  "all cols `v0.1.6:deployments` are present in `v1.0-rc1:deployments`", {
    ...
    dp <- suppressMessages(read_camtrap_dp(file = dp_path))
    cols_deployments_dp_v1_rc1 <- dp_v1_rc1$data$deployments %>%
      names()
    cols_deployments_dp_v0_1_6 <- dp$data$deployments %>%
      names()
...
test_that("read observations v1.0-rc1: media-based observations are removed", {
  expect_true(all(is.na(dp_v1_rc1$data$observations$mediaID)))
})

test_that("read observations v1.0-rc1: eventID is renamed as sequenceID", {
  expect_false("eventID" %in% names(dp_v1_rc1$data$observations))
  expect_true("sequenceID" %in% names(dp_v1_rc1$data$observations))
})

test_that("read observations v1.0-rc1: eventStart is renamed as timestamp", {
  expect_false("eventStart" %in% names(dp_v1_rc1$data$observations))
  expect_true("timestamp" %in% names(dp_v1_rc1$data$observations))
})

test_that(
  "read observations v1.0-rc1: eventEnd is a new term and is ignored", {
  expect_false("eventEnd" %in% names(dp_v1_rc1$data$observations))
})

test_that(
  "read observations v1.0-rc1: observationLevel is a new term and is ignored", {
    expect_false(
      "observationLevel" %in% names(dp_v1_rc1$data$observations)
    )
})
...
test_that(
  ...
  expect_false(
    "cameraSetupType" %in% names(dp_v1_rc1$data$observations)
  )
  expect_true("cameraSetup" %in% names(dp_v1_rc1$data$observations))
})

test_that("read observations v1.0-rc1: countNew is left empty", {
  expect_true(all(is.na(dp_v1_rc1$data$observations$countNew)))
})

test_that("read observations v1.0-rc1: higher taxonomic ranks ignored", {
  expect_false(
    any(c("kingdom", "phylum", "class", "order", "family", "genus") %in% 
          names(dp_v1_rc1$data$observations)
      )
  )
})
...
test_that(
  ...
  expect_false("behavior" %in% names(dp_v1_rc1$data$observations))
  expect_true("behaviour" %in% names(dp_v1_rc1$data$observations))
})
...
test_that(
  ...
  expect_false(
    "observationComments" %in% names(dp_v1_rc1$data$observations)
  )
  expect_true("comments" %in% names(dp_v1_rc1$data$observations))
})

test_that("read observations v1.0-rc1: _id is left empty", {
  expect_true(all(is.na(dp_v1_rc1$data$observations$`_id`)))
})

test_that(
  "read observations v1.0-rc1: individualSpeed is renamed as speed", {
    expect_false(
      "individualSpeed" %in% names(dp_v1_rc1$data$observations)
    )
    expect_true("speed" %in% names(dp_v1_rc1$data$observations))
})
...
test_that(
  "read observations v1.0-rc1: individualPositionRadius is renamed as radius", {
    ...
    expect_true("radius" %in% names(dp_v1_rc1$data$observations))
  }
)
...
test_that(
  "read observations v1.0-rc1: individualPositionAngle is renamed as angle", {
    ...
    expect_true("angle" %in% names(dp_v1_rc1$data$observations))
  }
)
...
test_that(
  "all cols `v0.1.6:observations` are present in `v1.0-rc1:observations`", {
    ...
    cols_obs_dp_v1_rc1 <- dp_v1_rc1$data$observations %>%
      dplyr::select(-dplyr::starts_with("vernacularNames")) %>%
      names()
    cols_obs_dp_v0_1_6 <- dp$data$observations %>%
      dplyr::select(-dplyr::starts_with("vernacularNames")) %>%
      names()
    expect_true(
...
test_that("read media v1.0-rc1: sequenceID is added", {
  expect_true("sequenceID" %in% names(dp_v1_rc1$data$media))
})

test_that(
  "read media v1.0-rc1: filePublic is a new term in v1.0-rc1 and is ignored", {
  expect_false("filePublic" %in% names(dp_v1_rc1$data$media))
})

test_that("read media v1.0-rc1: favorite is renamed as favourite", {
  expect_false("favorite" %in% names(dp_v1_rc1$data$media))
  expect_true("favourite" %in% names(dp_v1_rc1$data$media))
})

test_that("read media v1.0-rc1: mediaComments is renamed as comments", {
  expect_false("mediaComments" %in% names(dp_v1_rc1$data$media))
  expect_true("comments" %in% names(dp_v1_rc1$data$media))
})

test_that("read media v1.0-rc1: _id is left empty", {
  expect_true(all(is.na(dp_v1_rc1$data$media$`_id`)))
})
...
test_that(
  "all cols `v0.1.6:media` are present in `v1.0-rc1:media`", {
    ...
    dp <- suppressMessages(read_camtrap_dp(file = dp_path))
    cols_media_dp_v1_rc1 <- dp_v1_rc1$data$media %>%
      names()
    cols_media_dp_v0_1_6 <- dp$data$media %>%
      names()
