...
test_that("output is a list of length 4", {
  ...
  dp_without_multimedia <- read_camtrap_dp(
    path = dp_path,
    multimedia = FALSE)
  assert_that(length(dp_without_multimedia) == 4)
})
...
test_that("multimedia arg influences only slot multimedia", {
  ...
  assert_that(is.null(dp_without_multimedia$multimedia))
  assert_that(!is.null(dp_with_multimedia$multimedia))
  # metadata are the same
  assert_that(identical(dp_with_multimedia$metadata,
...
test_that("Datapackage metadata is a list", {
  ...
  dp_without_multimedia <- read_camtrap_dp(
    path = dp_path,
    multimedia = FALSE)
  assert_that(class(dp_without_multimedia$metadata) == "list")
})
...
test_that("Datapackage resources are named as in metadata$resource_names", {
  ...
  assert_that(all(resource_names %in% names(dp_without_multimedia)))
})
...
test_that("Datapackage resources are tibble dataframes", {
  ...
  dp_without_multimedia <- read_camtrap_dp(
    path = dp_path,
    multimedia = FALSE)
  assert_that(all(c("tbl_df", "tbl", "data.frame") %in% 
