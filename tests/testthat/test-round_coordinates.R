...
test_that("round_coordinates() sets lat, long, uncertainty and precision", {
  ...
  mica_gradients$data$deployments$longitude[[2]] <- 5.65
  mica_gradients$data$deployments$latitude[[2]] <- 51.18 # 30 latitude
  mica_gradients$data$deployments$coordinateUncertainty[[2]] <- NA_integer_
  mica_gradients$data$deployments$longitude[[3]] <- 5.65
  mica_gradients$data$deployments$latitude[[3]] <- -61.18 # 60 latitude
  mica_gradients$data$deployments$coordinateUncertainty[[3]] <- NA_integer_
  mica_gradients$data$deployments$longitude[[4]] <- 5.65
  mica_gradients$data$deployments$latitude[[4]] <- -85.18 # 85 latitude
  mica_gradients$data$deployments$coordinateUncertainty[[4]] <- NA_integer_

  mica1 <- round_coordinates(mica_gradients, 1)
  ...
  mica2 <- round_coordinates(mica_gradients, 2)
  ...
  mica3 <- round_coordinates(mica_gradients, 3)
...
test_that("round_coordinates() doesn't overestimate uncertainty on multiple runs", {
  ...
  mica_uncertainty <- mica
  mica_uncertainty$data$deployments$coordinateUncertainty[[1]] <- 10
  mica_uncertainty$data$deployments$coordinateUncertainty[[2]] <- NA_integer_

  mica1 <- round_coordinates(mica_uncertainty, 1)
  mica1_via_2 <- round_coordinates(round_coordinates(mica_uncertainty, 2), 1)
  expect_equal(
