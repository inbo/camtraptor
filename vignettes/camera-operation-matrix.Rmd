...
mica1$data$deployments$start <- deployments(mica1)$start[1]
mica1$data$deployments$end <- deployments(mica1)$deployments$end[1]
...
mica2 <- mica
mica2$data$deployments$locationName[4] <- deployments(mica2)$locationName[3]
mica2$data$deployments$start[4] <- deployments(mica2)$end[3] + ddays(5)
mica2$data$deployments$end[4] <- deployments(mica2)$start[4] + ddays(5)
deployments(mica2) %>% dplyr::select(locationName, start, end)
```
...
days_to_show <- seq(as.Date(deployments(mica2)$start[3]),
                    as.Date(deployments(mica2)$end[4]),
                    by = "days")
