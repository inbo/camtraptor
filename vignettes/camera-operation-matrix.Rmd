...
By loading package `camtrapdp`, a camera trap data package called `mica` is made available. This data package contains camera trap data of musk rats and coypus. We will use this variable from now on.
...
The matrix rows are the location names, by default the values of field `locationName` of `deployments`. Column names are dates. The matrix values are: 0 if station is not or only partially active, an integer (1 or more) if one or more deployments linked to the same station are fully active during the whole day.

In the example above there was a one-to-one relation between deployments and locations: the daily effort was 0 or 1.
...
mica1$deployments$start <- mica1$deployments$start[1]
mica1$deployments$end <- mica1$deployments$end[1]
...
days_to_show <- seq(as.Date(mica2$deployments$start[3]),
                    as.Date(mica2$deployments$end[4]),
                    by = "days")
...
```
### Assign decimal values to partially active days

The use of decimal figures to express partial active days is possible by setting argument `use_decimal` to `TRUE`. Here below a basic example where partial active days, i.e. `start` and `end` days, result in decimal values:

```{r basic_example_raw_decimal}
cam_op_matrix_decimals <- get_cam_op(mica, use_decimal = TRUE)
cam_op_matrix_decimals[1:4,c(as.character(as.Date(mica$deployments$start)),
                             as.character(as.Date(mica$deployments$end)))]
```

Notice that `get_cam_op()` with option `use_decimal` equal `TRUE` is internally used by `get_custom_effort()` to calculate custom efforts. 
