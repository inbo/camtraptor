...
x2 <- x
x2$data$deployments$locationName[4] <- deployments(x2)$locationName[3]
x2$data$deployments$start[4] <- deployments(x2)$end[3] + ddays(5)
x2$data$deployments$end[4] <- deployments(x2)$start[4] + ddays(5)
deployments(x2) %>% dplyr::select(locationName, start, end)
```
