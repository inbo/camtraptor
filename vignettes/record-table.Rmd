...
camtrapR_recordTable(x_dependent, 
                 minDeltaTime = 10, 
...
Similar to `recordTable()`, the function `camtrapR_recordTable()` allows you also to
exclude some species. Both vernacular names and scientific names are allowed (case insensitive):

```{r example_exclude}
camtrapR_recordTable(x, exclude = c("grey heron", "Anas platyrhynchos", "mens"))
```
...
camtrapR_recordTable(x, pred_gt("latitude", 51.18))
```
