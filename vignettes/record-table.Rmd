...
```{r example_exclude}
get_record_table(camtrapdp, exclude = c("BROWN rat", "Anas platyrhynchos"))
```

### Station names

The column containing the station names can also be defined by the user if the default value, `"location_name"`, is not the correct one. It has to be a valid column of  `deployments`. Here below, `location_id` is used:
...
As for visualization and all other functions, you can select a subset of deployments by using filter predicates. E.g. to get the record table of observations for the deployments with longitude equal or higher than 3.6:

```{r example_filter_predicates_map}
get_record_table(camtrapdp, pred_gt("longitude", 3.6))
```
