...
By default the color palette and the radius are defined based on the actual feature values. However, sometimes can be useful to set up an absolute upper limit. This can be done by setting argument  `relative_scale` to  `FALSE` and specifying the upper limit in  `max_scale`.
