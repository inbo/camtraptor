...
This vignette shows how to use function `map_deployments()` to visualize important deployment features on a leaflet map: 
...
You can visualize the number of species detected by each deployment by using the function `map_deployments()` with `feature` argument set to `n_species`:
...
# Create data package with three deployments with observations of unknown
# species and one deployment with 0 observations (its deploymentID is not
# present in observations table).

# Deployment didn't detect any known species
...
# Create the map
map_deployments(unknown_species_vs_no_obs,
        feature = "n_species")
```
